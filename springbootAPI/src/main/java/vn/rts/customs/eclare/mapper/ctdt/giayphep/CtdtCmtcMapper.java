...
import vn.rts.customs.eclare.dto.body.electronic_doc.CertificateContentDto;
...
@Mapper
public interface CtdtCmtcMapper {
	CtdtCmtcMapper INSTANCE = Mappers.getMapper(CtdtCmtcMapper.class);

	@Mappings({ @Mapping(source = "maPlHinhThucChungTu", target = "typeCode"),
			@Mapping(source = "soChungTu", target = "refNo"),
			@Mapping(source = "ngayPhatHanhChungTu", target = "issueDate", qualifiedByName = "formatToDate"),
			@Mapping(source = "nganhNgheKinhDoanh", target = "businessType"),
			@Mapping(source = "coSoPhapLy", target = "legal"), @Mapping(source = "ghiChuKhac", target = "content") })
	CertificateContentDto.Certificate ctdtCmtcToCertificateContentDto(CtdtCmtcEntity ctdtCmtcEntity);
	...
	CertificateContentDto chungTuDinhKemToKhaiToCertificateContentDto(ChungTuDinhKemTkEntity chungTuDinhKemTkEntity);
