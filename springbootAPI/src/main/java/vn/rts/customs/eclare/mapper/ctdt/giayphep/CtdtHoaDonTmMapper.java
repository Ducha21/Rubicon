...
import vn.rts.customs.eclare.dto.body.electronic_doc.InvoiceContentDto;
...
@Mapper
public interface CtdtHoaDonTmMapper {
	CtdtHoaDonTmMapper INSTANCE = Mappers.getMapper(CtdtHoaDonTmMapper.class);

	@Mappings({ @Mapping(source = "soHoaDonTm", target = "invoiceNo"),
			@Mapping(source = "ngayPhatHanhHoaDonTm", target = "invoiceDate", qualifiedByName = "formatToDate"),
			@Mapping(source = "maPlHinhThucHoaDon", target = "typeCode"),
			@Mapping(source = "tongGiaTriHoaDon", target = "totalInvoice"),
			@Mapping(source = "donViTienThanhToan", target = "invoiceCurrency"),
			@Mapping(source = "ghiChuKhac", target = "content") })
	InvoiceContentDto.Invoice ctdtHoaDonTmToInvoiceContentDto(CtdtHoaDonTmEntity ctdtHoaDonTmEntity);
	...
	InvoiceContentDto chungTuDinhKemToKhaiToInvoiceContentDto(ChungTuDinhKemTkEntity chungTuDinhKemTkEntity);
