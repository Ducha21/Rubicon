...
import vn.rts.customs.eclare.dto.body.electronic_doc.ValuationDeclarationContentDto;
...
@Mapper
public interface CtdtTktgMapper {
	CtdtTktgMapper INSTANCE = Mappers.getMapper(CtdtTktgMapper.class);
	...
	ValuationDeclarationContentDto chungTuDinhKemTkToValuationDeclarationContentDto(
			ChungTuDinhKemTkEntity chungTuDinhKemTkEntity);

	@Mappings({ @Mapping(source = "ghiChuKhac", target = "content") })
	ValuationDeclarationContentDto.ValuationDeclaration ctdtTktgToValuationDeclarationContentDto(
			CtdtTktgEntity ctdtTktgEntity);
